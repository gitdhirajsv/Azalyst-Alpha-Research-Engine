...
set LAST_DONE=0
if exist "checkpoint.json" (
    echo  [INFO] Found checkpoint.json - reading progress...
    for /f "tokens=2 delims=:," %%a in ('findstr /i "cycle_index" checkpoint.json') do (
        set LAST_DONE=%%a
        set LAST_DONE=!LAST_DONE: =!
    )
    echo  [INFO] Last completed step: !LAST_DONE!
)
...
if !LAST_DONE! GTR 1 (
    ...
    echo {"cycle_index": 1, "last_step": "step1_features", "timestamp": "%date% %time%"} > checkpoint.json
    set LAST_DONE=1
...
if !LAST_DONE! GTR 2 (
    ...
    echo {"cycle_index": 2, "last_step": "step2_walkforward", "timestamp": "%date% %time%"} > checkpoint.json
    set LAST_DONE=2
...
if !LAST_DONE! GTR 3 (
    ...
    echo {"cycle_index": 3, "last_step": "step3_factors", "timestamp": "%date% %time%"} > checkpoint.json
    set LAST_DONE=3
...
if !LAST_DONE! GTR 4 (
    ...
    echo {"cycle_index": 4, "last_step": "step4_statarb", "timestamp": "%date% %time%"} > checkpoint.json
    set LAST_DONE=4
...
if !LAST_DONE! GTR 5 (
    ...
    echo {"cycle_index": 5, "last_step": "step5_ml", "timestamp": "%date% %time%"} > checkpoint.json
    set LAST_DONE=5
...
if !LAST_DONE! GTR 6 (
    ...
    echo {"cycle_index": 6, "last_step": "step6_benchmark", "timestamp": "%date% %time%"} > checkpoint.json
    set LAST_DONE=6
...
if !LAST_DONE! GTR 7 (
    ...
    echo {"cycle_index": 7, "last_step": "step7_reports", "timestamp": "%date% %time%"} > checkpoint.json
    set LAST_DONE=7
...
if !LAST_DONE! GTR 8 (
    ...
    if errorlevel 1 (
        ...
        echo {"cycle_index": 8, "last_step": "step8_orchestrator", "timestamp": "%date% %time%"} > checkpoint.json
        set LAST_DONE=8
