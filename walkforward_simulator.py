...
BUY_THRESHOLD  = 0.60      # p_up > 0.60 → BUY
SELL_THRESHOLD = 0.40      # p_up < 0.40 → SELL
...
class PaperTradeEngine:
    ...
    def simulate(
        ...
        """
        Simulate one trade. Returns a trade dict or None if price data missing.
        """
        if signal == "HOLD":
            return None
        ...
        try:
            ...
            if len(future_bars) < self.horizon:
                return None
            ...
            # Exit bar (horizon bars after entry)
...
class WalkForwardSimulator:
    ...
    def run(self, resume: bool = True) -> None:
        ...
        self._log(f"  LightGBM     : {'Yes' if _LGBM else 'No (sklearn fallback)'}")
        self._log("═" * 65 + "\n")
        ...
        while predict_end <= global_end + pd.Timedelta(days=1):
            ...
            for ts, group in pred_df_ranked.groupby(level=0):
                ...
                for i, (_, row_data) in enumerate(valid_rows.iterrows()):
                    ...
                    if prob > BUY_THRESHOLD:
                        ...
                        signal = "HOLD"
                        # Still log the prediction for learning_log
                        # (but don't execute a trade)
            ...
            train_start = train_start + pd.Timedelta(days=self.predict_days)
            train_end   = predict_end
            predict_end = train_end + pd.Timedelta(days=self.predict_days)
            cycle_idx  += 1
